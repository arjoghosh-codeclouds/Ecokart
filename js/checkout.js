...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("./data/coupons.json")
    .then((res) => {
      if (!res.ok) throw new Error("Failed to fetch coupons.json");
      return res.json();
    })
    .then((data) => {
      coupons = data;
  ...
  function renderCheckout() {
    try {
      ...
      items.forEach((item) => {
        ...
        itemDiv.className = "flex justify-between items-center border-b pb-2 dark:text-gray-100";
